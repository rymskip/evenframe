...
mod tests {
    ...
    fn render(field_type: FieldType, value: Value) -> String {
        to_surreal_string(&field_type, &value, &ForeignTypeRegistry::default())
    }
    ...
    #[test]
    fn scalars_render_as_surql_literals() {
        assert_eq!(render(FieldType::String, json!("it's")), r"'it\'s'");
        assert_eq!(render(FieldType::Bool, json!(true)), "true");
        assert_eq!(render(FieldType::I64, json!(42)), "42");
        assert_eq!(render(FieldType::Unit, json!(null)), "null");
    }

    #[test]
    fn record_links_render_through_type_record() {
        assert_eq!(render(link(), json!("user:1")), "type::record('user:1')");
        assert_eq!(
            render(link(), json!({ "Id": "user:2" })),
            "type::record('user:2')"
        );
        assert_eq!(
            render(link(), json!({ "id": "user:3", "name": "Ada" })),
            "type::record('user:3')"
        );
    }

    #[test]
    fn containers_render_their_items_by_type() {
        let links = FieldType::Vec(Box::new(link()));
        assert_eq!(
            render(links, json!(["user:1", "user:2"])),
            "[type::record('user:1'), type::record('user:2')]"
        );
        assert_eq!(
            render(FieldType::Option(Box::new(link())), json!(null)),
            "null"
        );
        let object = FieldType::Struct(vec![
            ("name".to_string(), FieldType::String),
            ("owner".to_string(), link()),
        ]);
        assert_eq!(
            render(object, json!({ "name": "a", "owner": "user:1" })),
            "{ name: 'a', owner: type::record('user:1') }"
        );
    }

    #[test]
    fn untyped_datetimes_render_as_datetime_literals() {
        assert_eq!(
            render(
                FieldType::Other("Unknown".to_string()),
                json!("2025-05-29T23:00:00Z")
            ),
            "d'2025-05-29T23:00:00Z'"
        );
    }
}
